...
class Settings:
  def __init__(self) -> None:
    ...
    self.micu_management_url = str(_value("micu_management_url", "AI_AUDIT_MICU_MANAGEMENT_URL", "https://www.micuapi.ai"))
    self.micu_inference_url = str(_value("micu_inference_url", "AI_AUDIT_MICU_INFERENCE_URL", self.default_codex_base_url))
    self.micu_management_token = str(os.environ.get("AI_AUDIT_MICU_MANAGEMENT_TOKEN") or "")
    self.micu_user_id = str(_value("micu_user_id", "AI_AUDIT_MICU_USER_ID", "78836"))
    self.micu_token_group = str(_value("micu_token_group", "AI_AUDIT_MICU_TOKEN_GROUP", "vip_2"))
